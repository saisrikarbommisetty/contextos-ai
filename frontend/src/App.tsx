...
export function App() {
  ...
  // Parse Project Workspace path: e.g. /project/:id or /project/proj-campus-connect-01
  ...
  if (isProjectView && currentProjectId) {
    ...
    breadcrumbs.push({ label: currentProjectId.includes('campus-connect') ? 'CampusConnect' : currentProjectId });
  ...
  return (
    <div className="min-h-screen bg-background text-slate-100 flex flex-row">
      ...
      <Sidebar
        ...
        onNavigate={navigate}
        onResumeGlobal={() => handleResumeClick('proj-campus-connect-01')}
      />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <TopBar
          ...
          onNavigate={navigate}
          onResumeClick={() => handleResumeClick(currentProjectId || 'proj-campus-connect-01')}
          showResumeBtn={true}
        />
        ...
        <div className="flex-1">
          {isProjectView && currentProjectId ? (
            ...
            <ProjectWorkspacePage
              projectId="proj-campus-connect-01"
              initialTab="graph"
              onBackToDashboard={() => navigate('/dashboard')}
              onResumeClick={handleResumeClick}
              onGenerateBriefClick={handleBriefClick}
            ...
            <div className="p-10 max-w-5xl mx-auto space-y-6">
              ...
              <p className="text-xs text-slate-400">Select an active project to generate an executive handover brief:</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { id: 'proj-campus-connect-01', name: 'CampusConnect' },
                  { id: 'proj-ai-research-02', name: 'AI Research Assistant' },
                  { id: 'proj-team-portal-03', name: 'Team Productivity Portal' },
                ].map((p) => (
                  <div key={p.id} className="p-5 rounded-2xl glass-card border border-border flex flex-col justify-between">
                    <h3 className="font-bold text-sm text-white mb-2">{p.name}</h3>
                    <button
                      onClick={() => handleBriefClick(p.id)}
                      className="w-full py-2 rounded-xl bg-brand-600/20 hover:bg-brand-600/30 text-brand-300 text-xs font-semibold border border-brand-500/30 transition-all"
                    >
